...
<SortableContext items={shownTasks.filter(task => !task.completed).map(task => task.id)} strategy={verticalListSortingStrategy}><ol className="task-list">{shownTasks.map((task, index) => <TaskRow key={task.id} task={task} index={index} update={update} edit={anchor => openTask(task.id, anchor)} today={today} disabled={busy} />)}</ol></SortableContext>
    ...
    {(selectedEvent || creatingEvent) && <EventComposer key={`${selectedEvent?.id || 'new'}:${editorSession}`} event={events.find(event => event.id === selectedEvent?.id) || selectedEvent} refreshing={calendarLoading} unavailable={Boolean(selectedEvent && !events.some(event => event.id === selectedEvent.id))} anchor={eventAnchor} calendars={displayCalendars} start={creatingEvent && eventDraft ? eventDraft.start : focus} end={creatingEvent ? eventDraft?.end : undefined} demo={demo} close={() => { setSelectedEvent(undefined); setCreatingEvent(false); }} saved={message => { setNotice(message); setRefreshVersion(value => value + 1); }} />}
