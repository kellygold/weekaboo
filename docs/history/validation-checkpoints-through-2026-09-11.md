...
Superseded by ../current-validation.md. Retained for failure and artifact provenance; older status statements are not current instructions.
...
| Static website | Free/open-source privacy-first copy, flexible tasks, context groups, bidirectional sync, proper attributed icons; 8 Chromium/WebKit viewport checks including 320px, local assets/audio, no external requests | Local preview only at http://127.0.0.1:5190. GitHub source link configured and tested; binary download URLs unset; website not deployed |
...
Public-source checkpoint: local commit `4090e70` saved only the audited 413-file manifest with a GitHub noreply email. Source and full Git history scan both found zero secrets. No remote/push/public repository yet; awaiting generation-plan confirmation for included audio. A scanner false positive on ordinary MCP planning prose was resolved by rephrasing that prose, without exclusions or weakening rules.

## Public source published — 11 September 2026

Kelly confirmed paid ElevenLabs coverage for the generation date, resolving the audio hold. The audited repository is public at https://github.com/kellygold/weekaboo; README/MIT license/source verified. GitHub secret scanning, push protection and private vulnerability reporting are enabled. Local source and full-history Gitleaks scans passed before push. Website now links GitHub in navigation and its source section; all eight Chromium/WebKit viewport checks pass including source-destination assertions. No website deployment, DNS changes, binary release or store submission. Private proof stays under output/publication-audit/.

## Android physical candidate — 11 September, 22:05 AEST

Source `45ede45`; signed APK SHA-256 `f86b1cb826b81e8ad5a4f1bef3ed8369e31057076b80c5efa4456f54dcf725d9`. `output/production-validation/android-tablet-current/receipt.json` records the reconnected Lenovo tablet on a normal Android user OS.

- Same-certificate in-place upgrade passed; pulled installed APK matches the candidate exactly. All three provider accounts remained connected. Normal export before/after matched all three tasks exactly, including the completed item. No account reset, credential export or uninstall.
- Wi-Fi disabled, full process restart: saved events and tasks remained visible, with saved-data status. Wi-Fi restored; manual refresh showed its loading state and the final refreshed timestamp. Evidence is visually reviewed screenshots; accessibility XML omitted WebView children after restart. This proves cache/recovery, not true token expiry or fresh consent.
- Actual release Microsoft SDK fallback instrumentation passed one test on the physical tablet (and one earlier on the emulator). Temporary test package removed. Current app has no WebView inspection socket on the normal user OS.
- Exact native notice inventory: 91 runtime artifact/project records, 153 files, zero unresolved. Signed APK/AAB contents and bundletool validation pass. Collector has ten passing regression tests; shared suite has 180 passing tests. Evidence: `android-license-evidence/`.
- Independent Claude review of immutable `45ede45` against `8e9e82f` completed in 209 seconds: no blocking findings in the Android dependency/acknowledgment/signing delta. Stale Android release documentation corrected. This does not resolve the earlier incomplete cancellation follow-up or certify whole-product readiness.

No live CRUD fixtures or fresh interactive consent were exercised in this upgrade slice. Previous provider evidence remains separate. Unique task-export copies on the tablet were removed after preserving private Mac evidence. Wi-Fi is restored and Weekaboo reopened. No release uploaded.

## Current platform refresh — 11 September, 22:18 AEST

Phone controls: fixed the New event label overflowing an icon-sized button and Schedule overlapping the group picker. Existing phone layout retained. Chromium and WebKit checks at 375/390px verify header/toolbar separation and reachable editor/account/settings actions; all **182 shared tests pass**, and the two WebKit phone checks pass separately. A first `--browser=all` run reported two Firefox launch failures because that browser is not installed; it is not counted as tested. Browser checks do not prove native keyboard or real consent behavior. Removed the task footer's obsolete prototype label.

- **Android:** final signed APK `3fb084b4430bb15451feae413022b45816b1d1664952a8ddac4257daeb1a12b0` installed in place; pulled installed bytes match, WebView inspection remains disabled, and normal UI task export matches the original three tasks exactly. Latest notice/bundletool checks pass. The earlier offline/MSAL physical proof above predates only these UI changes; not claimed as a second offline/consent run. Receipt: `android-tablet-current/final-upgrade.json`.
- **iOS:** `ios-20260911T121155Z/matrix.json`: iPhone, iPad Mini and standard iPad each pass six core/UI tests (18 total), runtime 26.5. Current Release archive exported with App Store distribution signing: IPA `50b8069b1fce153370697b2ca1dce872ab8ebabba12f7b9c073776c4ff83b98d`. Signature/profile, disabled debugging/logging, 92 renderer resources and 13 native notice files match. Managed signing resolved missing local distribution material in a separate earlier-archive probe; the final export needed no provisioning updates. Reproducible command in ios-release.md. No upload or new physical iOS test.
- **Mac:** current signed DMG `8d5afee50b0e78d27eec388adebbbb23ea7a1284bac88ad23c02e9174ad50898` matches the signed app and all 74 renderer files; excludes Android-only supplements. Signed package smoke and isolated install/replacement/task export/import lifecycle pass. Fixed the DMG script to honor the signing identity consistently with the app packager, preventing accidental selection of a stale unsigned app. Still not notarized or checked through downloaded-file Gatekeeper.

All receipts remain private under output/production-validation or output/standalone-desktop. Remaining gates are in release-checklist.md; these passes do not establish production readiness or publication approval.
